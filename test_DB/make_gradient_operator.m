...
Divergence_t = Gt';
%% overlap nodes
[overlap_node] = find_overlap_all_node(Nx,Ny,nx,ny,'NODE'); % do not contain nodes on the boundary
...
[real_node_boundary,real_node_inside,real_node_useful,real_node_not_useful] =find_useful_and_not_useful_node_index(...
                                            Nx,Ny,Nnode,...
                                            nx,ny,nnode,...
...
[~,index_overlap_and_not_useful,~]=intersect(overlap_node(:,1),real_node_not_useful);
%% the index of nodes are ghost and not useful(nodes belongs to small mesh)
[~,index_ghostX_and_not_useful,~]=intersect(ghostx_node(:,1),real_node_not_useful);
[~,index_ghostY_and_not_useful,~]=intersect(ghosty_node(:,1),real_node_not_useful);
[~,index_ghostC_and_not_useful,~]=intersect(ghostc_node(:,1),real_node_not_useful);
...
Divergence(real_overlap_node(:,2),:) = Divergence(real_overlap_node(:,2),:) + Divergence(real_overlap_node(:,1),:);

...
interpolate_tianchong_small = phi_mr;
interpolate_tianchong_small = reshape(interpolate_tianchong_small,numel(interpolate_tianchong_small),1);
interpolate_tianchong_small(real_overlap_node(:,1)) = interpolate_tianchong_small(real_overlap_node(:,2));%test

interpolate_tianchong_small(real_ghostx_node(:,1)) = interpolate_tianchong_small(real_ghostx_node(:,2)).*...
                                                                                    real_ghostx_node_topo(:,2);%test
interpolate_tianchong_small(real_ghostx_node(:,1)) = interpolate_tianchong_small(real_ghostx_node(:,1)) +...
                                                                interpolate_tianchong_small(real_ghostx_node(:,3)).*...
                                                                                    real_ghostx_node_topo(:,3);%test

interpolate_tianchong_small(real_ghosty_node(:,1)) = interpolate_tianchong_small(real_ghosty_node(:,2)).*...
                                                                                    real_ghosty_node_topo(:,2);%test
interpolate_tianchong_small(real_ghosty_node(:,1)) = interpolate_tianchong_small(real_ghosty_node(:,1)) +...
                                                                interpolate_tianchong_small(real_ghosty_node(:,3)).*...
                                                                                    real_ghosty_node_topo(:,3);%test

interpolate_tianchong_small(real_ghostc_node(:,1)) = interpolate_tianchong_small(real_ghostc_node(:,2)).*...
                                                                                    real_ghostc_node_topo(:,2);%test
interpolate_tianchong_small(real_ghostc_node(:,1)) = interpolate_tianchong_small(real_ghostc_node(:,1)) +...
                                                                interpolate_tianchong_small(real_ghostc_node(:,3)).*...
                                                                                    real_ghostc_node_topo(:,3);%test
interpolate_tianchong_small(real_ghostc_node(:,1)) = interpolate_tianchong_small(real_ghostc_node(:,1)) +...
                                                                interpolate_tianchong_small(real_ghostc_node(:,4)).*...
                                                                                    real_ghostc_node_topo(:,4);%test
interpolate_tianchong_small(real_ghostc_node(:,1)) = interpolate_tianchong_small(real_ghostc_node(:,1)) +...
                                                                interpolate_tianchong_small(real_ghostc_node(:,5)).*...
                                                                                    real_ghostc_node_topo(:,5);%test
interpolate_tianchong_small = reshape(interpolate_tianchong_small(Nnode+1:end),nx+1,ny+1);


phi_big = reshape(phi1(1:Nnode),Nx+1,Ny+1);
phi_small = reshape(phi1(Nnode+1:end),nx+1,ny+1);
% phi_minus = abs((phi_big - phi_small(1:ratio:end,1:ratio:end))./...
%                                         phi_small(1:ratio:end,1:ratio:end));

phi_small_mr_minus_small = abs((phi_small_mr - phi_small)./phi_small)*100;
...
plotfield2d(phi_small_mr_minus_small,[0 0],dx,dy);
% plotfield2d(phi_minus,[0 0],Dx,Dy);

plotfield2d(interpolate_tianchong_small,[0 0],dx,dy);
last_minus = abs((interpolate_tianchong_small - phi_small)./phi_small)*100;
plotfield2d(last_minus,[0 0],dx,dy);

big_minus_big_mr = abs((phi_big - phi_big_mr)./phi_big)*100;
plotfield2d(big_minus_big_mr,[0 0],Dx,Dy);
end

...
%% truely overlapped or be ghosted
    node_big_control((left_pos_at_big):(right_pos_at_big+1),[(forward_pos_at_big),(backward_pos_at_big+1)]) = 19;
    node_big_control([(left_pos_at_big),(right_pos_at_big+1)],(forward_pos_at_big):(backward_pos_at_big+1)) = 19;
%% reshape and find the indexes
    node_big_control = reshape(node_big_control,(Nx_big+1)*(Ny_big+1),1);
    
    real_big_node_useful  = find( node_big_control < 19 );
    real_big_node_overlap_or_ghost = find(node_big_control == 19);
    real_big_node_boundary  = find(node_big_control == 0);
    real_big_node_inside  = find(node_big_control >0 & node_big_control < 19);
    real_big_node_not_useful = find( node_big_control >= 19 );
end
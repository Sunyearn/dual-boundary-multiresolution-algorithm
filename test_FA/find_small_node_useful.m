...
%                             ((forward_pos_at_big-1)*ratio_y+1):ratio_y:(backward_pos_at_big*ratio_y+1) ) = 19;
%     node_small_control( ((left_pos_at_big-1)*ratio_x+1):ratio_x:(right_pos_at_big*ratio_x+1),...
%                             [((forward_pos_at_big-1)*ratio_y+1),(backward_pos_at_big*ratio_y+1)] ) = 19;
    node_small_control( [((left_pos_at_big-1)*ratio_x+1),(right_pos_at_big*ratio_x+1)],...
                            ((forward_pos_at_big-1)*ratio_y+1):(backward_pos_at_big*ratio_y+1) ) = 19;
    node_small_control( ((left_pos_at_big-1)*ratio_x+1):(right_pos_at_big*ratio_x+1),...
                            [((forward_pos_at_big-1)*ratio_y+1),(backward_pos_at_big*ratio_y+1)] ) = 19;
...
%% reshape and find the indexes
    ...
    real_small_node_not_useful  = find(node_small_control < 19);
    real_small_node_boundary  = find(node_small_control < 0);
    real_small_node_inside  = find(node_small_control >= 19);
    real_small_node_useful = find(node_small_control >= 19);
    real_small_node_overlap_or_ghost = find(node_small_control == 19);
